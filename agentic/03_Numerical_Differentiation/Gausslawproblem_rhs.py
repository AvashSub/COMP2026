...
def make_figure(hs, errs):
    q, sigma = 1.0, 1.0
    E = gaussian_blob(q, [0, 0, 0], sigma)
    rho = gaussian_density(q, [0, 0, 0], sigma)
    Rs = np.linspace(0.3, 5.0, 18)
    lhs = np.array([sphere_flux(E, R, n_theta=200, n_phi=400) for R in Rs])
    rhs = np.array([volume_divergence(E, R) for R in Rs])

    fig, (a1, a2, a3) = plt.subplots(1, 3, figsize=(14, 4))

    a1.loglog(hs, errs, 'o-')
    a1.axvline(H_OPT, ls='--', c='k', lw=1)
    a1.text(H_OPT * 1.3, max(errs) * 0.2, r'$\epsilon^{1/3}$')
    a1.set_xlabel('step $h$ (log scale)'); a1.set_ylabel(r'$|\nabla\cdot E - \rho|$ (log scale)')
    a1.set_title('truncation vs roundoff')

    a2.plot(Rs, lhs, 'o', ms=5, label=r'LHS  $\oint E\cdot dA$')
    a2.plot(Rs, rhs, 'x', ms=7, label=r'RHS  $\int \nabla\cdot E\,dV$')
    a2.plot(Rs, enclosed_charge(q, sigma, Rs), 'k-', lw=1.2, label=r'exact $Q_{\rm enc}$')
    a2.set_xlabel('$R/\\sigma$'); a2.set_ylabel('charge'); a2.legend()
    a2.set_title('both sides vs the exact answer')

    a3.semilogy(Rs, np.abs(lhs - rhs), 'o-')
    a3.set_xlabel('$R/\\sigma$'); a3.set_ylabel('|LHS - RHS|')
